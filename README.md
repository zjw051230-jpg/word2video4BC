...
当前工具版本：`2.2.0`。首次安装会把官方仓库、main 分支和本地 Git 工作副本记录到工作区隐藏目录 `.word2video4BC`。以后无需再次提供链接，只要对 Codex 说“更新视频工具”，Codex 就会在该工作副本执行 `git pull --ff-only origin main`，再只更新工具白名单。

环境资源不足时先使用已安装 env4BC；否则只接受带 SHA-256 的经批准本地 env4BC 包。仍无法安全补齐则立即停止并提示联系维护人员。禁止从第三方站点或其他在线压缩包下载环境包。
...
```text
D:\视频生成\
├─ 视频本体\                 # 可复用工具、全局 Skills、索引、运行日志、模板
└─ 项目资源\                 # 项目、素材、生成结果、交付、快照、私有 API、缓存
```

为保证既有项目、技能和脚本不失效，根目录的 `1.projects`、`2.submission`、`3.skills`、`4.apis`、`5.summary`、`6.snapshot`、`tmp`、`task-log.jsonl` 与 `New-VideoProject.ps1` 均保留为兼容映射。它们不再保存第二份真实数据。Git 始终只管理本体工具包，绝不管理项目资源。
